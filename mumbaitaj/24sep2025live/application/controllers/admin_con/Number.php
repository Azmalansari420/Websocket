...
class Number extends CI_Controller
{
	...
	protected $arr_values = array(
						   	...
						   	'pagination_limit'=>'15',
						   	'formURL'=>'admin_con/number/UpdateData',
						   ); 
   ...
   //--check user login or not
    ...
    function UpdateData()
	{
		...
		if(isset($_POST['submit']))
		{
			...
			$numertime = $this->db->get_where('game_times',array('id'=>$game_time_id))->result_object();

			if(!empty($numertime))
			{
				$gametime = $numertime[0]->gametime;
				$stringtime = $numertime[0]->stringtime;
				$gettimeid = $numertime[0]->game_time_id;
			}

			$number = $this->input->post('number');			
			$create_on = $this->input->post('create_on');	
			$timestamp = strtotime($create_on);

			$status = $this->input->post('status');			
			$getdatetime = date('Y-m-d H:i:s');

			$randno = $this->input->post('randno');
			// if(empty($randno)) {
			//    $randno = rand(99999,999999); 
			// }
...
//---------------------status
	...
	public function getalltime()
	{
		...
		$html = "<option>--Select Game Time--</option>";
		$getall = $this->db->get_where('game_times', ['game_id' => $game_id])->result_array();

		foreach ($getall as $data) 
		{
		    $html .= "<option value='".$data['id']."'>".date("h:i A", strtotime($data['gametime']))."</option>";
		}

		$data['status'] = 200;
		$data['optionlist'] = $html;
		echo json_encode($data);
	}
