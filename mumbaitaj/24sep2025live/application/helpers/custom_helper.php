...
{
  $ci = & get_instance();
  $size = $ci->crud->selectDataByMultipleWhere('game',array('id'=>$id,));
  if(!empty($size))
  {
    $name = $size[0]->name;
  }
...
function gametime($id)
{
  $ci = & get_instance();
  $size = $ci->crud->selectDataByMultipleWhere('all_times',array('id'=>$id,));
  if(!empty($size))
  {
    $name = date("h:i A",strtotime($size[0]->time));
  }
  else
  {
    $name = "Not Found..";
...
function sendWebSocketMessage($message) {
    try {
        $client = new Client("wss://tirangacab.com:8080");
        // $client = new Client("ws://127.0.0.1:8080");
        $client->send($message);
        $client->close();
        return true;
    } catch (Exception $e) {
        log_message('error', "WebSocket send failed: ".$e->getMessage());
        return false;
    }
}
