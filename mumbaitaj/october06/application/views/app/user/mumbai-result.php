            ...
            <thead>
                <tr>
                    ...
                    <?php foreach($times as $t): ?>
                        <th><?= date("h:i A", strtotime($t->gametime)) ?></th>
                    <?php endforeach; ?>
                </tr>
            </thead>
            <tbody>
...
$(document).on("change", "#month, #year select", function () {
    $.ajax({
        ...
        success: function (res) 
        {
            ...
            // Time ko ab column bana rahe hain
            res.times.forEach(function (t) {
                html += `<th>${formatTime24to12(t.gametime)}</th>`;
            });
            ...
            for (let d = 1; d <= 31; d++) {
                ...
                res.times.forEach(function (t) {
                    ...
                    res.numbers.forEach(function (row) {
                        if (parseInt(row.game_time_id) === parseInt(t.game_time_id) && parseInt(row.day_no) === d) {
                            val = row.number;
                        }
                    });
                    html += `<td>${val}</td>`;
