            ...
            <thead>
                <tr>
                    ...
                    <?php foreach($times as $t): ?>
                        <th><?= gamename($t->game_id) ?> <br><?= date("h:i A", strtotime($t->gametime)) ?></th>
                    <?php endforeach; ?>
                </tr>
            </thead>
            <tbody>
...
$(document).on("change", "#month, #year select", function () {
    $.ajax({
        ...
        success: function (res) 
        {
            ...
            // Time + Game name ko column bana diya
            res.times.forEach(function (t) {
                html += `<th>${t.game_name}`+`<br>`+` ${formatTime24to12(t.gametime)}</th>`;
            });

            html += `</tr></thead><tbody>`;

            // Date ko ab row bana diya
            for (let d = 1; d <= 31; d++) {
                html += `<tr><td>${d}</td>`;
