...
function car_type_master($id)
{
  $ci = & get_instance();
  $size = $ci->crud->selectDataByMultipleWhere('car_type_master',array('id'=>$id,));
  if(!empty($size))
  {
    $name = $size[0]->name;
  }
...
function car_brandname($id)
{
  $ci = & get_instance();
  $size = $ci->crud->selectDataByMultipleWhere('car_brand',array('id'=>$id,));
  if(!empty($size))
  {
    $name = $size[0]->name;
  }
...
function car_name($id)
{
  $ci = & get_instance();
  $size = $ci->crud->selectDataByMultipleWhere('car_name',array('id'=>$id,));
  if(!empty($size))
  {
    $name = $size[0]->name;
  }
...
function car_status($value)
{
  $string = "";
  if($value==1)
  {
    $string = '<p class="mb-0 text-white font-weight-bold d-flex justify-content-start align-items-center">
                  <small>
                    <svg class="mr-2" xmlns="http://www.w3.org/2000/svg" width="18" viewBox="0 0 24 24" fill="none">        
                      <circle cx="12" cy="12" r="8" fill="#3cb72c"></circle>
                    </svg>
  ...
  {
    $string = '<p class="mb-0 text-info-emphasis font-weight-bold d-flex justify-content-start align-items-center">
                  <small>
                     <svg class="mr-2" xmlns="http://www.w3.org/2000/svg" width="18" viewBox="0 0 24 24" fill="none">
                        <circle cx="12" cy="12" r="8" fill="#087990"></circle>
                     </svg>
                  </small>Pending
               </p>';
  }
 ...
 function calculate_fare($meters, $ratePerKm)
  {
    $km = $meters / 1000;
    $fare = $km * $ratePerKm;
    return round($fare, 2);
  ...
  function calculate_percentage($amount, )
    {
        $value = ($amount * 5) / 100;
  ...
  function date_time_formet($date)
    {
      $value = "--";
      $value = date('d-m-Y h:i:A',strtotime($date));
      return $value; 
    }
...
{
  $ci = & get_instance();
  $name = $ci->crud->selectDataByMultipleWhere('users',array('id'=>$id,));  
  return $name;
}



function car_booking_status($value)
{
  $string = "";
  if($value==1)
  {
