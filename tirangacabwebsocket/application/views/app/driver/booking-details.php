...
$booking_notifications = $booking_notifications = $this->db->get_where('booking_notifications',['id' =>$id,])->result_object();

if(!empty($booking_notifications))
{
  $booking_notifications = $booking_notifications[0];
  $bookings = $this->db->get_where('bookings', array('id' => $booking_notifications->booking_id))->result_object(); 
    if(!empty($bookings))
    {
      $bookings = $bookings[0];
    ...
    $drivername = get_userdata(@$booking_notifications->accepted_driver_id);
    // print_r($drivername[0]->name);die;
}
